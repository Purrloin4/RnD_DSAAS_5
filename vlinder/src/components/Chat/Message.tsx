...
export default function Message({ message }: { message: Imessage }) {
  ...
  return (
    <div className={`flex gap-2 items-start ${isOwnMessage ? "justify-end" : "justify-start"}`}>
      ...
      <div
        className={`max-w-[70%] p-3 rounded-lg ${
          isOwnMessage ? "bg-blue-500 text-white self-end" : "bg-gray-500 text-gray-300 self-start"
        }`}
      >
        {/* Username and Message Metadata */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            {!isOwnMessage && <h1 className="font-bold">{message.profiles?.username}</h1>}
