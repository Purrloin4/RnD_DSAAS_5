...
export default function Page() {
  ...
  const fetchProfile = async () => {
    ...
    const { data: profileHobbiesData, error: profileHobbiesError } =
      await supabase
    ...
    if (profileHobbiesError) {
      console.error(
        "Failed to fetch profile hobbies:",
        profileHobbiesError.message
      );
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage(
        "Profile saved successfully! You will be redirected to the homepage."
      );
  ...
  return (
    <main className="w-full min-h-screen flex flex-col justify-start items-center p-4">
      ...
      <div className="w-full max-w-md h-fit">
        ...
        <Button
          className="w-full mt-8 text-black btn-primary font-semibold"
          onClick={handleSave}
          disabled={loading}
        >
